...
echo
until grep -q '服务端域名与端口' "$HOME/cfs5http/$port.log" && grep -q '客户端地址与端口' "$HOME/cfs5http/$port.log" && grep -q '运行中的优选IP' "$HOME/cfs5http/$port.log"; do sleep 1; done; grep -m1 '服务端域名与端口\|客户端地址与端口\|运行中的优选IP' "$HOME/cfs5http/$port.log"
echo
elif [ "$menu" = "3" ]; then
showmenu
